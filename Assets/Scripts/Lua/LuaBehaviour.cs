...
public class LuaBehaviour : MonoBehaviour
{
	...
	{
		object result = Lua.RequireLua( LuaScriptPathName );
		DebugEx.Assert( result != null, string.Format( "Lua controller file must return something! ({0})", LuaScriptPathName ) );
		LuaFunction rf = result as LuaFunction;
		DebugEx.Assert( rf != null, string.Format( "Lua controller file must return a function! ({0})", LuaScriptPathName ) );
		...
		{
			DebugEx.LogError( string.Format( "{0} {1}", e.Source, e.Message ) );
			return;
		}
#endif
		DebugEx.Assert( results.Length > 0, string.Format( "Lua controller class must return something! ({0})", LuaScriptPathName ) );

		_selfTable = results[0] as LuaTable;
		DebugEx.Assert( _selfTable != null, string.Format( "Lua controller class must return a table! ({0})", LuaScriptPathName ) );

		_setGameObjectFunc = _selfTable["setGameObject"] as LuaFunction;
		DebugEx.Assert( _setGameObjectFunc != null );
